...
test/
  ...
  ├─ regression.test.mjs 실기기 버그 회귀 14건
...
node test/regression.test.mjs # 실기기 버그 회귀 14건
...
배포 후 폰에서 **"버튼을 눌러도 반응이 없거나 늦다"** 는 증상이 나왔습니다.

원인은 `render()`가 매번 `bind()`를 다시 호출한 것이었습니다.
`render()`는 `#root`의 `innerHTML`만 갈아끼우고 **`#root` 자체는 그대로 둡니다.**
그래서 리스너가 지워지지 않고 계속 쌓였습니다.

```
렌더 10회 → 리스너 11개 → 탭 한 번에 핸들러 11번 실행
```

토글 동작(칩 선택 등)이 짝수 번 실행되면 **제자리로 돌아와 아무 일도 안 일어난 것처럼 보입니다.**
그리고 매 실행이 또 재렌더를 부르니 기하급수적으로 느려집니다.

**`bind()`는 `init()`에서 한 번만 호출합니다.** 이벤트 위임을 쓰는 이상 그것으로 충분합니다.

함께 고친 것들:

- **스크롤 위치 보존** — 같은 화면 안의 재렌더면 스크롤을 유지합니다.
  칩 하나 눌렀다고 맨 위로 튕기면 누른 항목이 화면 밖으로 사라져 "안 눌렸다"고 느낍니다
- **포커스·커서 복원** — 입력 중이던 필드로 되돌아갑니다
- **IME 조합 가드** — 한글 조합 중 `innerHTML`을 갈아끼우면 글자가 깨지므로 조합이 끝날 때까지 렌더를 미룹니다
- **`touch-action: manipulation`** — 더블탭 확대 대기(약 300ms)를 없앱니다
- **입력 필드 16px** — iOS는 16px 미만 입력에 포커스하면 화면을 확대하고 되돌아오지 않습니다

`test/regression.test.mjs`가 이 다섯 가지를 전부 검증합니다.

### 9. 다국어 — 번역 비용을 3층으로 나눈다
